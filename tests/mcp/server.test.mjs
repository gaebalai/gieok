// server.test.mjs — gieok-mcp 서버의 JSON-RPC E2E 테스트
//
// stdio에 initialize → initialized → tools/list → tools/call을 파이프하여 응답을 검증.
...
before(async () => {
  workspace = await mkdtemp(join(tmpdir(), 'gieok-mcp-srv-'));
...
describe('gieok-mcp server', () => {
  test('MCP1 initialize returns serverInfo', async () => {
    ...
    assert.equal(init.result?.serverInfo?.name, 'gieok-wiki');
  });

  test('MCP2 tools/list returns 8 gieok_ tools', async () => {
    const child = startServer();
    const responses = await rpc(child, [
      {
        jsonrpc: '2.0',
  ...
  test('tools/call gieok_read returns wiki content', async () => {
    const child = startServer();
    const responses = await rpc(child, [
      {
        jsonrpc: '2.0',
      ...
      { jsonrpc: '2.0', method: 'notifications/initialized' },
      {
        ...
        id: 2,
        method: 'tools/call',
        params: { name: 'gieok_read', arguments: { path: 'index.md' } },
      },
    ]);
  ...
  test('MCP12 tools/call invalid args returns error result', async () => {
    ...
    const responses = await rpc(child, [
      ...
      { jsonrpc: '2.0', method: 'notifications/initialized' },
      {
        ...
        id: 2,
        method: 'tools/call',
        params: { name: 'gieok_read', arguments: { path: '../../etc/passwd' } },
      },
    ]);
    const callResp = responses.find((r) => r.id === 2);
    assert.ok(callResp);
  ...
  test('stdout is JSON-RPC only (no console.log noise)', async () => {
    const child = startServer({ GIEOK_DEBUG: '1' });
    let stderrOut = '';
    ...
    assert.match(stderrOut, /gieok-mcp/);
  });
