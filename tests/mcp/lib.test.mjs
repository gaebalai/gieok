// lib.test.mjs — mcp/lib/*의 단위 테스트
//
// 실행: node --test tools/claude-brain/tests/mcp/
...
async function makeVault() {
  const root = await mkdtemp(join(tmpdir(), 'gieok-mcp-lib-'));
...
describe('lock', () => {
  ...
  test('LIB31 withLock removes lockfile after completion', async () => {
    ...
    await assert.rejects(stat(join(vault, '.gieok-mcp.lock')));
  });
  ...
  test('LIB33 writeSummaryLock writes `.gieok-summary-<key>.lock` with pid+timestamp', async () => {
    // v0.3.5 Option B: detached claude -p 관찰용 lockfile
    const key = 'papers--test-pdf';
    const lockPath = await writeSummaryLock(vault, key, 12345);
    assert.equal(lockPath, join(vault, '.gieok-summary-papers--test-pdf.lock'));
...
describe('templates', () => {
  ...
  test('LIB52 honors GIEOK_TEMPLATES_DIR override', async () => {
    const overrideDir = await mkdtemp(join(tmpdir(), 'tpl-'));
    await writeFile(
      join(overrideDir, 'concept.md'),
      '---\ntitle:\ntags: [override]\n---\n\n## Custom\n',
    );
    const prev = process.env.GIEOK_TEMPLATES_DIR;
    process.env.GIEOK_TEMPLATES_DIR = overrideDir;
    try {
      const t = await loadTemplate('concept');
      assert.deepEqual(t.data.tags, ['override']);
      assert.match(t.body, /## Custom/);
    } finally {
      if (prev === undefined) delete process.env.GIEOK_TEMPLATES_DIR;
      else process.env.GIEOK_TEMPLATES_DIR = prev;
