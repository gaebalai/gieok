// tools-write-wiki.test.mjs — gieok_write_wiki 핸들러의 단위 테스트
...
before(async () => {
  // 템플릿을 실 프로젝트에서 참조
  process.env.GIEOK_TEMPLATES_DIR = TEMPLATES_DIR;
});

beforeEach(async () => {
  workspace = await mkdtemp(join(tmpdir(), 'gieok-mcp-ww-'));
...
after(async () => {
  delete process.env.GIEOK_TEMPLATES_DIR;
});
