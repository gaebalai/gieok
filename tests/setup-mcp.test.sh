#!/usr/bin/env bash
#
# setup-mcp.test.sh — scripts/setup-mcp.sh의 스모크 테스트 (Phase M)
#
# 실행: bash tools/claude-brain/tests/setup-mcp.test.sh
#
# 검증 항목:
#   MCP27  node 부재 시 exit 1 + 안내 메시지
#   MCP28  --dry-run에서 npm install을 호출하지 않음 (출력에 [dry-run] 마크 있음)
#   MCP28b npm 부재 시 exit 1
#   MCP28c --help로 usage 출력
...
# -----------------------------------------------------------------------------
# MCP27: node 부재 시 exit 1 (PATH에 /bin /usr/bin만 남겨 node를 보이지 않게 함)
# -----------------------------------------------------------------------------
