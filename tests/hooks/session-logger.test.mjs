// session-logger.test.mjs — hooks/session-logger.mjs 의 유닛 테스트
//
// 실행: node --test tools/claude-brain/tests/hooks/
//
// 원칙:
//   - 실제 Vault 를 건드리지 않는다 (mktemp -d)
//   - 네트워크 없음
//   - 테스트 종료 시 tmpdir 을 확실히 삭제
//   - session_id 는 test-session-* 의 고정 프리픽스
...
describe('session-logger: UserPromptSubmit', () => {
  ...
  test('title truncated to 50 code points preserving surrogate pairs', async () => {
    const { root, vault } = await createVault();
    try {
      const prompt = '긴 타이틀의 테스트입니다'.repeat(10);
      await runHook(vault, {
  ...
  test('ghost session: PostToolUse without prior UserPromptSubmit creates no file', async () => {
    ...
    try {
      ...
      // 고스트만 있는 상태에서는 index.json 자체가 미생성 (null) 이거나, 있어도 해당 sid 가 없음
      assert.ok(index === null || !index.sessions || !index.sessions['test-session-0013']);

      // 같은 session_id 로 Stop 이 와도 마찬가지
      await runHook(vault, {
        session_id: 'test-session-0013',
      ...
      // 나중에 UserPromptSubmit 이 오면 정상적으로 파일이 생성됨
      await runHook(vault, {
        session_id: 'test-session-0013',
        hook_event_name: 'UserPromptSubmit',
        cwd: '/tmp',
...
// R4-004: GIEOK_NO_LOG 와 cwd-in-vault 의 테스트
describe('session-logger: recursion guards', () => {
  test('GIEOK_NO_LOG=1 suppresses all output', async () => {
    const { root, vault } = await createVault();
    try {
      const p = {
      ...
      const { code } = await runHook(vault, p, { GIEOK_NO_LOG: '1' });
