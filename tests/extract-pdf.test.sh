#!/usr/bin/env bash
#
# extract-pdf.test.sh — scripts/extract-pdf.sh 의 결합 테스트.
#
# 실행: bash tools/claude-brain/tests/extract-pdf.test.sh
#
# 전제:
#   - poppler (pdfinfo / pdftotext) 가 설치되어 있음
#     macOS: brew install poppler  /  Debian: apt install poppler-utils
#   - tests/fixtures/pdf/*.pdf 가 commit 되어 있음
#
# 검증 항목 (설계서 26041705 §8.1):
#   EP1  8p PDF → 분할되지 않고 1 파일 (pp001-008.md)
#   EP2  42p PDF → 3 chunks (pp001-015 / pp015-030 / pp030-042) + overlap 검증
#   EP3  암호화 PDF → exit 2 + ERROR 로그
#   EP4  스캔 이미지 PDF → exit 3 + WARN 로그
#   EP5  본문에 AWS_ACCESS_KEY_ID 가 포함된 PDF → 출력에서 마스킹됨
#   EP6  pdfinfo Title 이 "Microsoft Word - *" → 폐기하고 파일명 기반 title
#   EP7  사이드카 .meta.yaml 로 Title 을 덮어쓰기
#   EP8  멱등성 — PDF 보다 새로운 chunk MD 가 있으면 스킵
#   EP9  soft limit 초과 → 선두 N 페이지만 + truncated: true
#   EP10 hard limit 초과 → exit 4 + 완전 스킵
#   EP11 사이드카 extract_layout: true → frontmatter extractor 가 -layout 포함
#   EP12 source_type sanitize — 쉘 메타 문자가 제거됨
#   EP16 (기능 2.1) sha256: chunk MD frontmatter 에 source_sha256 (64hex) 이 기록됨
#   EP17 (기능 2.1) 신 명명: chunk MD 는 `<subdir>--<stem>-pp*.md` (이중 하이픈)
...
# 실행 환경의 OBSIDIAN_VAULT 를 분리하고, 각 테스트 케이스에서 필요에 따라 설정한다.
# 사용자 쉘의 실제 Vault 를 참조하여 realpath prefix match 가 실패하는 것을 방지한다.
...
# 전제 체크: poppler 가 없으면 거의 모든 테스트가 의미가 없으므로 조기 skip
...
# VULN-007 (P4) 대책: exit 3 시에는 중도 chunk MD 를 삭제함
...
set +e
out9="$(GIEOK_PDF_MAX_SOFT_PAGES=10 GIEOK_PDF_MAX_HARD_PAGES=20 \
  bash "${EXTRACT}" \
...
set +e
out10="$(GIEOK_PDF_MAX_HARD_PAGES=10 \
  bash "${EXTRACT}" \
...
# -----------------------------------------------------------------------------
# EP14: OBSIDIAN_VAULT 가 설정되어 있을 때, 가짜 raw-sources (Vault 외부) 를 reject
# VULN-011 의 회귀 테스트 — substring `*/raw-sources/*` 만으로는 통과해버린다
# `/tmp/attacker/raw-sources/evil.pdf` 스타일의 경로를 prefix match 로 차단한다.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# EP15: OBSIDIAN_VAULT 가 설정되어 있을 때, Vault 내의 raw-sources/ 는 허용됨
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# EP16 (기능 2.1): sha256 frontmatter 검증 — chunk MD 에 source_sha256 (64hex) 이 기록됨
#                  + 같은 PDF 를 재실행했을 때 sha256 이 일치 (멱등성)
# -----------------------------------------------------------------------------
...
# 64 hex 의 source_sha256 행을 정규식으로 확인
...
# 재실행 시, 수동으로 계산한 sha256 과 동일한지 (멱등)
...
# -----------------------------------------------------------------------------
# EP17 (기능 2.1): 신 명명 검증 — chunk MD 는 `<subdir>--<stem>-pp*.md` (이중 하이픈)
# -----------------------------------------------------------------------------
...
# 구 명명 (papers-a-b-pp*.md, subdir 와 stem 사이가 단일 하이픈) 은 생성되지 않음
