...
Claude Code 사용자는 플러그인 마켓플레이스 슬래시 명령으로 한 번에 설치 가능 (Claude Code 대화창에 직접 입력):
...
│   ├── marketplace.json             ← `/plugin marketplace add gaebalai/gieok` 엔트리
...
- **Claude Code 플러그인 마켓플레이스 (C-2)** — `.claude-plugin/marketplace.json` + `plugin.json` 추가. Claude Code 대화창에서 `/plugin marketplace add gaebalai/gieok` 후 `/plugin install gieok@gaebalai-marketplace` 로 설치 가능
