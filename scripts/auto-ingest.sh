...
# -----------------------------------------------------------------------------
# Raw MD sha256 계산 헬퍼 (v0.6 Phase C-3)
# -----------------------------------------------------------------------------
# raw-sources/<subdir>/<name>.md (fetched/ 이외, 사용자가 직접 배치한 MD) 의 source_sha256 을
# 결정한다. 우선순위:
#   1. raw MD 의 frontmatter 에 `source_sha256: "<64hex>"` 가 있으면 해당 값 반환
#   2. 없으면 file binary 의 sha256 을 shasum -a 256 (macOS/BSD) 또는
#      sha256sum (Linux) 으로 계산하여 반환
#   3. 둘 다 사용 불가하면 빈 문자열 (판정 불가 = 기존 동작)
# PDF chunk / EPUB 챕터 / DOCX / fetched/ 는 이미 source_sha256 을 frontmatter 에 가지므로
# 이 헬퍼는 사용하지 않는다 (기존 .cache/extracted/ loop 과 fetched/ branch 에서 별도 awk).
compute_raw_md_sha() {
  local file="$1"
  local fm_sha
  fm_sha="$(awk -F'"' '
...
# -----------------------------------------------------------------------------
# Raw MD sha256 sidecar 생성 (v0.6 Phase C-3)
# -----------------------------------------------------------------------------
#
# raw-sources/<subdir>/<name>.md (fetched/ 이외) 의 sha256 을 .cache/raw-md-sha/ 에
# sidecar 로 기록한다. LLM 은 `--allowedTools Write,Read,Edit` 로 기동되어
# shasum 을 실행할 수 없다. raw MD 에 source_sha256 frontmatter 가 없는 경우에도
# 요약에 올바른 sha256 을 설정할 수 있도록, cron 이 여기서 계산한 값을 sidecar 로 전달.
#
# 명명 규약: .cache/raw-md-sha/<subdir>-<flat_name>.sha256
#   - <flat_name> 은 raw MD 의 subdir 하위 경로 (예: nested/foo.md) 의 `/` 를 `-` 로
#     치환한 형태 (raw-sources MD 집계 루프의 flat_name 과 같은 규약)
#   - summary 파일명 (wiki/summaries/<subdir>-<flat_name>) 과 1:1 대응
#
# 내용: 64hex + 개행 1개의 1줄 텍스트. LLM 이 Read 로 읽어 그대로 source_sha256 에
# 복사할 수 있는 최소 포맷.
#
# 삭제: raw MD 가 삭제되어도 sidecar 는 남는다 (후속 PR 에서 GC 검토). 현재는 해 없음
# (orphan sidecar 는 summary 쪽 sha256 과 대조되지 않을 뿐).
...
if [[ -d "${RAW_SOURCES_DIR}" ]]; then
  ...
  while IFS= read -r f; do
    ...
    [[ "${rel}" != */* ]] && continue
    name="${rel#*/}"
    # fetched/ 하위는 extract-url.sh 가 frontmatter 에 source_sha256 을 쓰므로 sidecar 불필요
...
if [[ -d "${RAW_SOURCES_DIR}" ]]; then
  while IFS= read -r f; do
    ...
    if [[ "${name}" == fetched/* ]]; then
      ...
      # v0.6 Phase C-3: fetched/ 이외의 raw MD (사용자 직접 배치 / handler 경유가 아닌 .md)
      # 에도 sha256 기반 delta detection 을 적용한다.
      # src_sha = raw MD 의 frontmatter source_sha256 이 있으면 그 값, 없으면 file binary
      #           sha256 (compute_raw_md_sha 가 2단계로 결정).
      # sum_sha = summary 의 frontmatter source_sha256.
      # 불일치 시 재 ingest 대상. LLM 은 후속 프롬프트에서 sidecar 를 통해 sha256 을 기록.
...
- **source_sha256 기록 (v0.6 Phase C-3)**: raw-sources/<subdir>/<name>.md 에서 summary 를 신규 작성하거나 갱신할 때는, 대응하는 wiki/summaries/<subdir>-<flat_name>.md 의 frontmatter 에 `source_sha256: "<64hex>"` 를 반드시 설정할 것. 값은 다음 순서로 결정한다:
  1. raw MD 의 frontmatter 에 `source_sha256: "<64hex>"` 가 있으면 그 값을 한 글자도 틀리지 않게 복사
  2. raw MD 에 frontmatter source_sha256 이 없으면, cron 이 사전 계산한 sidecar `.cache/raw-md-sha/<subdir>-<flat_name>.sha256` 을 Read 로 읽어 내용의 64hex (말미 개행은 제거) 를 복사. `<flat_name>` 은 raw MD 의 subdir 하위 경로의 `/` 를 `-` 로 치환한 형태 (예: `nested/foo.md` → `nested-foo.md`)
  3. sidecar 도 찾지 못한 경우 source_sha256 을 설정하지 않음 (멱등 판정이 동작하지 않아 매번 re-ingest 되지만 부작용은 없음)
  이 source_sha256 은 다음 cron 의 멱등 판정에 사용되므로 정확히 복사할 것. raw MD 의 내용이 바뀌지 않는 한 같은 값이 재사용되어 LLM 의 쓸데없는 호출을 방지. fetched/ 하위 MD 는 기존대로 raw MD 의 frontmatter 에서 직접 복사 (sidecar 는 생성되지 않음)
