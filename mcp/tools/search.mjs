// gieok_search — Wiki 검색.
// qmd CLI에 위임 (BM25 / vector / hybrid), qmd 부재 시에는 Node grep의 간이 fallback.
...
export const SEARCH_TOOL_DEF = {
  name: 'gieok_search',
  title: 'Search GIEOK wiki',
  description:
    'Search the GIEOK wiki via qmd (BM25/vector/hybrid). Falls back to a simple Node grep if qmd is not installed. ' +
    'For best results when both servers are connected, prefer the dedicated qmd MCP `query` tool — gieok_search is a thin wrapper kept here so wiki search works even without qmd.',
  inputShape: {
...
async function tryQmd(query, limit, mode) {
  ...
  return new Promise((resolve) => {
    const child = spawn('qmd', args, {
      ...
      env: { ...process.env, GIEOK_NO_LOG: '1' },
    });
    let stdout = '';
    let stderr = '';
    let done = false;
    ...
    child.on('error', () => {
      if (!done) {
        ...
        clearTimeout(timer);
        resolve(null); // ENOENT (qmd not installed) 등
      }
    });
    child.on('close', (code) => {
      if (done) return;
