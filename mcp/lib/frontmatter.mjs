// frontmatter.mjs — YAML frontmatter 의 최소 서브셋 parse / serialize / merge.
// Node 18+ stdlib 만. Wiki 템플릿이 사용하는 형식 (flat key + scalar / array) 만 지원.
...
function stripQuotes(s) {
  if (s.length >= 2) {
    // Double-quoted: JSON.parse 로 \uXXXX / \n / \" 등의 이스케이프를 복호한다.
    // 복호에 실패하는 경우 (ill-formed JSON) 는 안전하게 raw slice 를 반환한다.
    // JSON double-quoted 는 YAML double-quoted 의 엄격한 서브셋이므로 안전.
    if (s[0] === '"' && s[s.length - 1] === '"') {
      try { return JSON.parse(s); } catch { return s.slice(1, -1); }
    }
    // Single-quoted: YAML 단일 따옴표는 단순 (유일한 이스케이프는 '' → ').
    if (s[0] === "'" && s[s.length - 1] === "'") {
